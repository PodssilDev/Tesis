...
# 5) Generar barplot
# Crear el gráfico
grafico_top10_vars <- ggplot(top10_vars, aes(x = Variable, y = Frecuencia, fill = Mediana_IncMSE)) +
...
ggsave(filename = "Resultados/Determinantes/Top_Variables_Frecuencia.png",
       plot = grafico_top10_vars,
       width = 10,
       height = 6,
       dpi = 300)

# Mostrar el gráfico en consola
print(grafico_top10_vars)
...
graficos_determinantes <- lapply(names(random_forest), function(anio) {
  ...
  g <- ggplot(df_imp, aes(x = reorder(Variable, IncMSE), y = IncMSE)) +
  ...
  archivo_salida <- paste0("Resultados/Determinantes/Determinantes_", anio, ".png")
  ggsave(filename = archivo_salida, plot = g, width = 8, height = 5, dpi = 300)
