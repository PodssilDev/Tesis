...
load("Resultados/Informes/informe.RDATA")   # carga variables guardadas (datos_procesados, etc.)
library(dplyr)
library(ggplot2)
library(knitr)
```
...
knitr::include_graphics("Resultados/Eficiencia/violinplot_eficiencia.png")
```

```{r violin-complejidad-img, echo=FALSE, out.width="100%"}
knitr::include_graphics("Resultados/Eficiencia/violinplot_eficiencia_complejidad.png")
```
...
# Carpeta donde están los mapas
carpeta_mapas <- "Resultados/Mapas/"

# Listar solo los archivos que coinciden con el patrón
mapas <- list.files(path = carpeta_mapas,
                    pattern = "Gráfica Chile - Eficiencia técnica_Año_.*\\.jpg$",
                    full.names = TRUE)

# Ordenar por nombre (para que se mantenga el orden por año)
mapas <- sort(mapas)
...
knitr::include_graphics("Resultados/Comparacion/Correlacion_SFA_DEA_OOVRS.png")
```

```{r distancia-vs, echo=FALSE, out.width="100%"}
knitr::include_graphics("Resultados/Comparacion/Distancia_Euclidiana_SFA_DEA_OOVRS.png")
```